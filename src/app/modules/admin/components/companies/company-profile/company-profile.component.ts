...
export class CompanyProfileComponent implements OnInit, OnChanges {
  ...
  async onFileChangeLogo(e) {
    const file = CompanyProfileComponent.validateFileUpload(e);
  ...
  onFileChangePhotos(e, name) {
    const files = CompanyProfileComponent.validateFiles(e);
    if (!files) {
      return null;
    }
    Object.keys(files)
  ...
  getPictureName(name: any = '', file: File) {
    if (file) {
      name = file.name.replace(
        'https://knac-assets.s3.us-west-1.amazonaws.com/',
        ''
      );
